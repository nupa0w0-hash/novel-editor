...
export function generateHTML(episode: NovelEpisode): string {
    ...
    if (header.heroImageLayout === 'background') {
      heroImageHTML = `
        <div style="position: relative; width: 100%; padding-bottom: ${paddingBottom}; background-image: url('${header.heroImageUrl}'); background-size: cover; background-position: center; margin-bottom: 2rem;">
          <div style="position: absolute; inset: 0; display: flex; align-items: ${getTitleAlignment(header.titlePosition || 'center').vertical}; justify-content: ${getTitleAlignment(header.titlePosition || 'center').horizontal}; padding: 2rem;">
            <div style="text-align: center; color: ${header.titleColor || '#fff'}; text-shadow: 0 2px 8px rgba(0,0,0,0.5);">
              <h1 style="font-size: 3rem; font-weight: 900; margin-bottom: 0.5rem;">${title}</h1>
              ${header.subtitle ? `<p style="font-size: 1.25rem; color: ${header.subtitleColor || '#fff'}; margin-bottom: 0.5rem;">${header.subtitle}</p>` : ''}
              ${header.author ? `<p style="font-size: 1rem; color: ${header.authorColor || '#fff'};">${header.author}</p>` : ''}
            </div>
          </div>
        </div>
      `;
      ...
      heroImageHTML = `
        <div style="width: 100%; padding-bottom: ${paddingBottom}; background-image: url('${header.heroImageUrl}'); background-size: cover; background-position: center; margin-bottom: 2rem;"></div>
      `;
    } else if (header.heroImageLayout === 'below') {
      heroImageHTML = `
        <div style="width: 100%; padding-bottom: ${paddingBottom}; background-image: url('${header.heroImageUrl}'); background-size: cover; background-position: center; margin-top: 2rem;"></div>
      `;
    }
  }
  ...
  const headerHTML = header.heroImageLayout !== 'background' || !header.heroImageUrl ? `
    <div style="text-align: center; margin-bottom: 3rem;">
      <h1 style="font-size: 2.5rem; font-weight: 900; margin-bottom: 0.5rem; color: ${header.titleColor || style.bodyText};">${title}</h1>
      ${header.subtitle ? `<p style="font-size: 1.125rem; color: ${header.subtitleColor || style.bodyText}; opacity: 0.7; margin-bottom: 0.5rem;">${header.subtitle}</p>` : ''}
      ${header.author ? `<p style="font-size: 0.875rem; color: ${header.authorColor || style.bodyText}; opacity: 0.6; font-weight: 700;">${header.author}</p>` : ''}
      ${header.tags && header.tags.length > 0 ? `<div style="display: flex; gap: 0.5rem; justify-content: center; margin-top: 1rem; flex-wrap: wrap;">${header.tags.map(tag => `<span style="background: ${style.highlightBg}; color: ${style.highlightText}; padding: 0.25rem 0.75rem; border-radius: 1rem; font-size: 0.75rem; font-weight: 700;">${tag}</span>`).join('')}</div>` : ''}
    </div>
  ` : '';

  // Generate body
  const bodyHTML = blocks.map(block => {
    const processedText = processDialogue(block.text, style.highlightBg, style.highlightText);
    return `<p style="margin-bottom: 1.5rem; line-height: ${style.lineHeight}; letter-spacing: ${style.letterSpacing}px;">${processedText}</p>`;
  }).join('\n');
...
function processDialogue(text: string, highlightBg: string, highlightText: string): string {
  // Replace Korean quotes and English quotes with highlighted spans
  return text
